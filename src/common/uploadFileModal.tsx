...
const UploadFileModal = ({ handleSubmit }: UploadFileModalProps) => {
  ...
  const parseXML = (xmlString: string) => {
    try {
      ...
      return 'errorMessages.errorInvalidXMLFormat'.toString();
    }
  };
  ...
  const validateFile = useCallback(
    async (fileToValidate: File): Promise<string | undefined> => {
      if (fileToValidate.size === 0) {
        return 'errorMessages.errorEmptyFile'.toString();
      }
      if (fileToValidate.type === 'application/json') {
        try {
          JSON.parse(await fileToValidate.text());
        } catch (e) {
          return 'errorMessages.errorInvalidJSONFormat'.toString();
        }
      } else if (
        ...
        return 'errorMessages.errorUnsupportedFileType'.toString();
      }
